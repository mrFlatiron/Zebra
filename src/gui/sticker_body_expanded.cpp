...
//  m_apply_pb->set_icon (style_utils::common_icons::check_mark);
  m_apply_pb = new QToolButton;
  m_apply_pb->setIcon (QIcon (get_icon_path (style_utils::common_icons::check_mark)));
  m_apply_toolbar = new QToolBar;
  m_apply_toolbar->addWidget (m_apply_pb);
...
void sticker_body_expanded::set_layout ()
{
  QVBoxLayout *vlo_0 = new QVBoxLayout;
  {
    ...
    {
      hlo_1->addStretch ();
      hlo_1->addWidget (m_apply_toolbar);
    }
...
//  m_conn.connect_to (m_apply_pb->clicked, [this] {this->on_apply_clicked ();});
  connect (m_apply_pb, &QToolButton::clicked, [this] {on_apply_clicked ();});
}
