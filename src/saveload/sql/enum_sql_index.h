...
class enum_sql_index
{
  const enum_sql_table<Enum> *m_table = nullptr;
  std::vector<Enum> m_indexed_columns;
  std::string m_index_name;
  bool m_if_not_exist_kw = false;
  bool m_unique_kw = false;
public:
  enum_sql_index () = default;
  virtual ~enum_sql_index () = default;

  explicit enum_sql_index (const enum_sql_table<Enum> *table)
  {
    m_table = table;
  }
...
public:
  ...
  void set_table (const enum_sql_table<Enum> *table)
  {
    m_table = table;
  }
